...
| **M3 — kofin v2** ✅ **code complete (same day)** | §6.2 adoption ported from the reference branch: negotiation, StateVersion + snapshot-on-gap, snapshot apply, beacons as drift reference, v2 reconnect/wake/resync, Hello probe + dedicated-socket time sync (probe measured 1ms RTT vs 20ms HTTP). kofin gates green (black/mypy/2,114 unit tests) + a 12/12 live probe of the real client classes against the plugin server (`plugin.video.kofin/tests/live/syncplay_v2_probe.py`) | kit's 3-user mixed scenarios + a real web+kofin watch session | **remaining: the human watch session on real Kodi** |
